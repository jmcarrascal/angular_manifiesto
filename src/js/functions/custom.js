$(document).ready(function() {
	...
	$("#wizard2").formwizard({
		...
		validationOptions : {
			...
			success : function(label) {
				label.text('Success!').addClass('valid').closest('.control-group').addClass('success');
			}
		}
	});
	...
	var elf = $('#file-manager').elfinder({
		...
		uiOptions : {
			...
			toolbar : [['back', 'forward'], ['info'], ['quicklook'], ['search']]
		},
		contextmenu : {
	...
	oTable = $('#data-table').dataTable({
		...
		"oLanguage" : {
			"sLengthMenu" : "<span>Cantidad de entradas:</span> _MENU_"
		}
	});
...
//	$('.typeahead').typeahead();
	...
	$(".ui-datepicker-month, .style, .dataTables_length select").uniform({
		radioClass : 'choice'
	});
